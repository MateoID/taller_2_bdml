...
test <- test[, .SD, .SDcols = c("id", variables_modelo)]
```
```{r}
...
```{r}
test_numeric <- test[, lapply(.SD, function(x) as.numeric(as.character(x))), .SDcols = variables_modelo[-1]]
test_matrix <- xgb.DMatrix(
  data = as.matrix(test_numeric[, variables_modelo[-1], with = FALSE])
)
