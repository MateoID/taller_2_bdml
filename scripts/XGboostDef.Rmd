...
xgb.save(final_xgb, "../models/xgboost_model.model")

# 2. Guardar modelo GLMNET
saveRDS(final_glmnet, "../models/glmnet_model.rds")

# 3. Guardar los parámetros del ensemble
ensemble_metadata <- list(
  ensemble_weight = avg_weight,
  best_threshold = best_threshold,
  feature_names = colnames(train_matrix),
  smote_params = list(K = 3, dup_size = 4),
  xgb_params = params,
  glmnet_params = list(alpha = 0.2, type.measure = "auc")
)

saveRDS(ensemble_metadata, "../models/ensemble_metadata.rds")

# 4. Guardar información de las variables (opcional, útil para análisis)
variable_info <- list(
  importance_matrix = importance_matrix,
  oof_performance = f1_oof,
  train_performance = best_f1
)

saveRDS(variable_info, "../models/variable_info.rds")

# 5. Guardar todo junto en un único archivo (alternativa)
save(
  final_xgb,
  final_glmnet,
  ensemble_metadata,
  variable_info,
  file = "../models/full_ensemble_model.RData"
)

cat("Modelos guardados exitosamente en la carpeta ../models/\n")
```
